...
namespace ServerTCPService
{
        ...
        public class TCPServer
        {
            ...
            /// <summary>
            /// An Event handler that capture new client request served
            /// </summary>
            private void socketListener_onClientServed(string request,string response,Socket clientSocket)
            {
                if(onClientServed != null)
                {
                    onClientServed(request,response,clientSocket);
                }
            }
            ...
            private void ServerThreadStart()
            {
                ...
                while (!m_stopServer)
                {
                    try
                    {
                        ...
                        if(onClientServed != null)
                        {
                            onClientServed("", "", clientSocket);
                        }
                        // Create a SocketListener object for the client.
                        socketListener = new TCPSocketListener(clientSocket);

                        socketListener.onClientServed += socketListener_onClientServed;
